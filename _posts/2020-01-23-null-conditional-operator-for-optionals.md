...
1. Depending on return type of $$f$$ EITHER $$op($$`std::optional<T>`$$,f_1)\rightarrow$$`std::optional<U>` if `U` is not an optional type,
 OR $$op($$`std::optional<T>`$$,f_1)\rightarrow$$`std::optional<V>` if `U` is an optional type of `V`.
