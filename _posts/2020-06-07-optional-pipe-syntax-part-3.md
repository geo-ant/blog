...
#description:
#permalink:
title: 'Implementing a Pipe Syntax for `std::optional` - Part 3: Intended Side Effects and Unintended Limitations'
comments_id: 12
---
...
In order to avoid making copies of the arguments in the `void` overload, I have chosen to return the initial argument by const reference. Now, let's make a minor modification to the code above and make `length` a const reference.
